    ...
    def get_children(self, obj):
        ...
        def get_plugin_data(child_plugin):
            SerializerClass = get_serializer_class(
                plugin=child_plugin.get_plugin_class())
            instance, plugin = child_plugin.get_plugin_instance()
            return SerializerClass(instance).data
...
class PlaceHolderSerializer(RequestSerializer, serializers.ModelSerializer):
    ...
    def get_plugins(self, obj):
        data = []
        for plugin in obj.get_plugins(self.language).filter(parent__isnull=True):
            SerializerClass = get_serializer_class(
                plugin=plugin.get_plugin_class())
            instance, __ = plugin.get_plugin_instance()
            data.append(SerializerClass(instance).data)
        return data
...
class LightPageSerializer(RequestSerializer, serializers.ModelSerializer):
